...
show_help() {
    ...
    echo ""
    echo "用法:"
    echo "  ./scripts/dev.sh          启动后端服务"
    echo "  ./scripts/dev.sh --help  显示此帮助信息"
    echo ""
    echo "说明:"
    echo "  - 后端服务：http://localhost:5025"
...
start_backend() {
    ...
    print_info "服务地址: http://localhost:5025"

    cd "$BACKEND_DIR"

    # 使用 dotnet run 启动
    dotnet run 2>&1 | sed -e "s/^/[${CYAN}BACKEND${NC}] /" &
    BACKEND_PID=$!
...
main() {
    ...
    echo ""
    echo -e "  ${GREEN}后端 API:${NC}      http://localhost:5025"
    echo ""
