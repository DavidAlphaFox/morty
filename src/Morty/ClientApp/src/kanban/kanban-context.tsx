...
import { fetchProjectStories, createStory as apiCreateStory, startStoryPhase, updateStory as apiUpdateStory, updateStoryRequirements, updateUserAcceptanceCriteria, regeneratePlan, regenerateAcceptance, rescheduleStory } from '../api/client';
...
export const KanbanProvider: ParentComponent = (props) => {
  ...
  const moveStoryToColumn = async (storyId: number, toColumn: KanbanColumnId) => {
    ...
    // 检查是否为重新调度（Completed/Failed → Pending）
    const currentStory = stories().find((s) => s.id === storyId);
    const isReschedule = currentStory &&
      (currentStory.phase === 'Completed' || currentStory.phase === 'Failed') &&
      toColumn === 'Pending';
    ...
    if (isReschedule) {
      ...
      try {
        const updated = await rescheduleStory(storyId);
        setStories((prev) => prev.map((s) => (s.id === storyId ? updated : s)));
      } catch (err) {
        console.error('Failed to reschedule story:', err);
        refreshStories();
      }
    } else {
      // 乐观更新
